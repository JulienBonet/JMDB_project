...
//     const [rows] = await db.query(`...`); // ta requête SQL ici
...
//     // Commande mysqldump SANS mot de passe dans la ligne de commande
...
//         env: {
//           ...process.env,
//           MYSQL_PWD: DB_PASSWORD, // <-- mot de passe fourni via variable d'environnement
//         },
//       },
//       (error, stdout, stderr) => {
//         if (error) {
  ...
  try {
    ...
    exec(
      cmd,
      {
        env: {
          ...process.env,
          MYSQL_PWD: DB_PASSWORD, // mot de passe via variable d'environnement
        },
      },
      (error, stdout, stderr) => {
        if (error) {
