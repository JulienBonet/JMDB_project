...
function MovieThema() {
  ...
  const handleSortedAlphabeticalFocus = async () => {
    const url = sortFocusAsc
      ? `${backendUrl}/api/casting/focus/sorted/0` // ASC
      : `${backendUrl}/api/casting/focus/sorted/1`; // DESC
  ...
  const handleSortedChronologicalFocus = async () => {
    const url = sortFocusAsc
      ? `${backendUrl}/api/casting/focus/sorted/2` // ASC
      : `${backendUrl}/api/casting/focus/sorted/3`; // DESC
  ...
  const handleClickFocus = async (f) => {
    ...
    const res = await fetch(`${backendUrl}/api/casting/${f.id}`);
    const data = await res.json();
    setFilms(data);
  };
  ...
  const handleSortedAlphabeticalMovies = async () => {
    ...
    const url = sortMoviesAsc
      ? `${backendUrl}/api/casting/${selectedFocus.id}/sorted/0` // ASC
      : `${backendUrl}/api/casting/${selectedFocus.id}/sorted/1`; // DESC
  ...
  const handleSortedChronologicalMovies = async () => {
    ...
    const url = sortMoviesYearAsc
      ? `${backendUrl}/api/casting/${selectedFocus.id}/sorted/2` // ASC
      : `${backendUrl}/api/casting/${selectedFocus.id}/sorted/3`; // DESC
  ...
  const handleResetMovies = async () => {
    ...
    const res = await fetch(`${backendUrl}/api/casting/${selectedFocus.id}`);
    const data = await res.json();
    setFilms(data);
