...
function AdminScreenwriterList() {
  ...
  useEffect(() => {
    getArtistsSortedById('screenwriters')
      .then((datas) => {
        setData(datas);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        const status = await deleteArtist('screenwriters', id);

        if (status === 204) {
          toast.success('screenwriter deleted', {
            className: 'custom-toast',
          });
          refreshScreenwriters();
        } else {
    ...
    <section className="AdminItemsSection">
      <section className="HeaderAdminItemsSection">
        ...
        <div className="admin_feat_tools_line">
          ...
          <Button variant="contained" onClick={() => openModalNewScreenWriter()}>
